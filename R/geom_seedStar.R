...
GeomSeedStar <- ggproto("GeomSeedStar", Geom,
                         required_aes = c("x", "y"),
                         ...
                         draw_panel = function(data, panel_params, coord) {
                             ...
                             class(grobs) <- "gList"
                             ggplot2:::ggname("geom_seedStar", gTree(children = grobs))
                         }
                         
)


##' a ggplot2 layer of star as a polygon
##'
##'
...
##' @examples
##' library(ggplot2)
##' ggplot(mtcars, aes(mpg, disp)) + geom_seedStar()
geom_seedStar <- function(mapping = NULL, data = NULL, stat = "identity",
                           position = "identity", na.rm = FALSE, show.legend = NA,
                           inherit.aes = TRUE, ...) {
  
    layer(geom = GeomSeedStar, mapping = mapping, data = data, stat = stat,
